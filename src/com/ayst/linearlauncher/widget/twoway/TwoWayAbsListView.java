...
public abstract class TwoWayAbsListView extends TwoWayAdapterView<ListAdapter>
    ...
    /**
     * The last scroll stop time
     */
    private long mLastScrollStopTime;

    /**
     * Max stay time / Unit(ms)
     */
    private static final int MAX_STAY_TIME = 25;
    ...
    public void setPadding(int left, int top, int right, int bottom) {
        ...
        mPaddingLeft = Math.max(mPaddingLeft, left);
        mPaddingRight = Math.max(mPaddingRight, right);
    }
...
//    	super.requestLayout();
    ...
    void keyPressed() {
        ...
        if (!shouldShowSelector()
                || (selector != null
                        && (isFocused() || touchModeDrawsInPressedState())
    ...
    void reportScrollStateChange(int newState) {
        if (newState != OnScrollListener.SCROLL_STATE_IDLE)
            removeCallbacks(mScrollIdleStateRunnable);
    ...
    Runnable mScrollIdleStateRunnable = new Runnable() {
        public void run() {
     ...
     */
    boolean resurrectSelectionIfNeed() {
        if (mSelectedPosition < 0 && mTouchHandler.resurrectSelection()) {
            updateSelecter();
            return true;
        }
        return false;
     ...
     *            in the recycler.
     * @see com.ktcp.launcher.widget.twoway.coship.vod.view.jess.ui.TwoWayAbsListView.RecycleBin
     * @see com.ktcp.launcher.widget.twoway.coship.vod.view.jess.ui.TwoWayAbsListView.RecyclerListener
     */
     ...
     * 
     * @see com.ktcp.launcher.widget.twoway.coship.vod.view.jess.ui.TwoWayAbsListView.RecycleBin
     * @see com.ktcp.launcher.widget.twoway.coship.vod.view.jess.ui.TwoWayAbsListView#setRecyclerListener(com.ktcp.launcher.widget.twoway.coship.vod.view.jess.ui.TwoWayAbsListView.RecyclerListener)
     */
    ...
    abstract class TouchHandler {
        ...
        /**
         * Fires an "on scroll state changed" event to the registered
         * {@link com.ktcp.launcher.widget.twoway.coship.vod.view.jess.ui.TwoWayAbsListView.OnScrollListener}
    ...
    class HorizontalTouchHandler extends TouchHandler {
            ...
            switch (action) {
                case MotionEvent.ACTION_DOWN: {
                    ...
                    if (touchMode != TOUCH_MODE_FLING && motionPosition >= 0) {
                        ...
                        // a fling).
                        // Remember where the motion event started
                        mPressed = getChildAt(motionPosition - mFirstPosition);
                        mMotionViewOriginalLeft = mPressed.getLeft();
                        mMotionX = x;
                        mMotionY = y;
                        mMotionPosition = motionPosition;
                        mTouchMode = TOUCH_MODE_DOWN;
                    ...
                    switch (mTouchMode) {
                        ...
                        case TOUCH_MODE_SCROLL:
                            ...
                            if (x != mLastX) {
                                ...
                                mLastScrollStopTime = SystemClock
                                        .elapsedRealtime();
                            }
                            break;
                    }
                ...
                case MotionEvent.ACTION_UP: {
                    switch (mTouchMode) {
                        ...
                        case TOUCH_MODE_DONE_WAITING:
                            ...
                            if (true /* child != null && !child.hasFocusable() */) {
                                if (mTouchMode != TOUCH_MODE_DOWN) {
                                    child.setPressed(false);
                                }
                            ...
                            if (childCount > 0) {
                                if (mFirstPosition == 0
                                    ...
                                    if (Math.abs(initialVelocity) > mMinimumVelocity) {
                                        ...
                                        final long diff = SystemClock
                                                .elapsedRealtime()
                                                - mLastScrollStopTime;
                                        mLastScrollStopTime = 0;
                                        if (diff <= MAX_STAY_TIME) {
                                            reportScrollStateChange(OnScrollListener.SCROLL_STATE_FLING);
                                            mFlingRunnable
                                                    .start(-initialVelocity >> 1);
                                        } else {
                                            reportScrollStateChange(OnScrollListener.SCROLL_STATE_IDLE);
                                        }
                                    } else {
        ...
        boolean resurrectSelection() {
            ...
            Log.i(TAG, "mResurrectToPosition " + mResurrectToPosition
            ...
            if (selectedPos >= firstPosition
                    && selectedPos <= getLastVisiblePosition()) {
                mLayoutMode = LAYOUT_SPECIFIC;
                mSpecificTop = Math.max(mSpecificTop, mListPadding.left);
                setSelectionInt(selectedPos);
                invokeOnItemScrollListener();
            } else {
                selectedPos = INVALID_POSITION;
        ...
        boolean trackMotionScroll(int delta, int incrementalDelta) {
            // if (DEBUG) Log.i(TAG, "trackMotionScroll() - deltaX: " + delta +
            ...
            final int spaceAbove = listPadding.left - firstLeft - mPaddingLeft;
            final int end = getWidth() - listPadding.right;
            final int spaceBelow = lastRight - end + mPaddingRight;
            ...
            if (down) {
                ...
                final int right = getWidth() - listPadding.right
                        - incrementalDelta + mPaddingRight;
                for (int i = childCount - 1; i >= 0; i--) {
                    final View child = getChildAt(i);
...
//            }
            ...
            if (spaceAbove < 0 || spaceBelow > 0 ||Math.abs(spaceAbove) < 15 || Math.abs(spaceBelow) < 15) {
                fillGap(down);
            }
