...
public abstract class TwoWayAdapterView<T extends Adapter> extends ViewGroup {
    ...
    private static class SelectionNotifier extends Handler implements Runnable {
        private WeakReference<TwoWayAdapterView> mView;

        public SelectionNotifier(TwoWayAdapterView view) {
            super();
            mView = new WeakReference<TwoWayAdapterView>(view);
        }

        public void run() {
            TwoWayAdapterView view = mView.get();
            view.handleHandler(this);
        }
    }

    private void handleHandler(SelectionNotifier handler) {
        if (mDataChanged) {
            // Data has changed between when this SelectionNotifier
            // was posted and now. We need to wait until the AdapterView
            // has been synched to the new data.
    ...
    private int mMyOldSelectPotion = -1;
    
    private OnItemUnSelectedListener mOnItemUnSelectedListener;

    public interface OnItemUnSelectedListener{
     void onItemUnSelected(View lose, int get);
    }
    
    public void setOnItemUnSelectedListener(OnItemUnSelectedListener l)
    {
    	mOnItemUnSelectedListener = l;
    }
    ...
    private void fireOnSelected() {
        ...
        {
        	mOnItemUnSelectedListener.onItemUnSelected(getUnSelectedView(mMyOldSelectPotion), selection);
        }
        mMyOldSelectPotion = selection;
        if (selection >= 0) {
    ...
    public View getUnSelectedView(int potion) {
        if (mItemCount > 0 && potion >= 0) {
            return getChildAt(potion - mFirstPosition);
        } else {
            return null;
        }
    }
